...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	for (i = 0; devlist[i]; i++) {
		...
		for (j = 0; j < num_analog_channels; j++) {
			...
			ch = sr_channel_new(sdi, j + num_logic_channels, SR_CHANNEL_ANALOG,
					TRUE, channel_name);
...
static int configure_channels(const struct sr_dev_inst *sdi)
{
	...
	for (l = sdi->channels, p = 0; l; l = l->next, p++) {
		...
		if ((p <= NUM_CHANNELS) && (ch->type == SR_CHANNEL_ANALOG)) {
			sr_spew("configure_channels");
			devc->ch_enabled[p] = ch->enabled;
			devc->enabled_analog_channels = g_slist_append(devc->enabled_analog_channels, ch);
		}
	}
